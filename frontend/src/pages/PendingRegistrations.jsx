              ...
              <AnimatePresence>
                {showFilters && (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Institution Type</label>
                      <select 
                        value={tempFilters.instType}
                        onChange={(e) => setTempFilters({...tempFilters, instType: e.target.value})}
                        className="bg-slate-950 border border-slate-800 text-slate-300 text-sm rounded-lg p-2 focus:outline-none focus:border-cyan-500/50"
                      >
              ...
              {Object.entries(activeFilters).map(([key, value]) => {
                ...
                return (
                  <span key={key} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-cyan-900/30 border border-cyan-500/30 text-cyan-400 text-xs font-medium">
                    {key === 'instType' ? 'Type: ' : key === 'time' ? 'Time: ' : 'Sector: '}
            ...
            <AnimatePresence>
              {isSearching && (
                ...
                >
                  ...
                  <p className="text-sm text-cyan-400 font-medium tracking-wide">Searching Registrations...</p>
                </motion.div>
              )}
            </AnimatePresence>
